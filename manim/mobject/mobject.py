...
class Mobject:
    ...
    def __init__(
        ...
        self.point_hash = None  # TODO: unused?
    ...
    def set_default(cls, **kwargs: Any) -> None:
        ...
        """
        if kwargs:
            cls.__init__ = partialmethod(cls.__init__, **kwargs)  # type: ignore
        else:
            cls.__init__ = cls._original__init__  # type: ignore
    ...
    def __deepcopy__(self, clone_from_id: dict[int, Any]) -> Self:
        cls = self.__class__
    ...
    def __add__(self, mobject: Mobject) -> Mobject:
    ...
    def __getattr__(self, attr: str) -> types.MethodType:
        ...
        if attr.startswith("set_"):
            ...
            def setter(self: Self, value: Any) -> Self:
                warnings.warn(
                    "This method is not guaranteed to stay around. Please prefer "
    ...
    def update(self, dt: float = 0, recursive: bool = True) -> Self:
        ...
        for updater in self.updaters:
            if "dt" in inspect.signature(updater).parameters:
                updater(self, dt)  # type: ignore
            else:
                updater(self)  # type: ignore
        if recursive:
    ...
    def get_time_based_updaters(self) -> list[TimeBasedUpdater]:
        ...
        return time_based_updaters  # type: ignore
    ...
    def add_updater(
        ...
        if call_updater:
            ...
            if "dt" in parameters:
                update_function(self, 0)  # type: ignore
            else:
                update_function(self)  # type: ignore
        return self
    ...
    def scale(
        self, scale_factor: float, **kwargs: Unpack[ApplyPointsFunctionKwargs]
    ) -> Self:
    ...
    def rotate(
        ...
        angle: float,
        axis: Vector3D = OUT,
        **kwargs: Unpack[ApplyPointsFunctionKwargs],
    ) -> Self:
    ...
    def flip(
        self, axis: Vector3D = UP, **kwargs: Unpack[ApplyPointsFunctionKwargs]
    ) -> Self:
    ...
    def stretch(
        self, factor: float, dim: int, **kwargs: Unpack[ApplyPointsFunctionKwargs]
    ) -> Self:
    ...
    def apply_function(
        self, function: MappingFunction, **kwargs: Unpack[ApplyPointsFunctionKwargs]
    ) -> Self:
    ...
    def apply_matrix(
        self, matrix: Matrix3x3, **kwargs: Unpack[ApplyPointsFunctionKwargs]
    ) -> Self:
        # Default to applying matrix about the origin, not mobjects center
    ...
    def repeat(self, count: int) -> Self:
        ...
        def repeat_array(array: np.ndarray) -> np.ndarray:
    ...
    def pose_at_angle(self, **kwargs: Unpack[ApplyPointsFunctionKwargs]) -> Self:
    ...
    def shift_onto_screen(self, **kwargs: Unpack[AlignOnBorderKwargs]) -> Self:
    ...
    def is_off_screen(self) -> bool:
        ...
        if self.get_bottom()[1] > config["frame_y_radius"]:
            return True
        top_y: float = self.get_top()[1]
        return top_y < -config["frame_y_radius"]
    ...
    def scale_to_fit_width(
        self, width: float, **kwargs: Unpack[ApplyPointsFunctionKwargs]
    ) -> Self:
    ...
    def scale_to_fit_height(
        self, height: float, **kwargs: Unpack[ApplyPointsFunctionKwargs]
    ) -> Self:
    ...
    def scale_to_fit_depth(
        self, depth: float, **kwargs: Unpack[ApplyPointsFunctionKwargs]
    ) -> Self:
    ...
    def space_out_submobjects(
        self, factor: float = 1.5, **kwargs: Unpack[ApplyPointsFunctionKwargs]
    ) -> Self:
    ...
    def put_start_and_end_on(self, start: Point3DLike, end: Point3DLike) -> Self:
        ...
        self.scale(scale_factor, about_point=curr_start)
        self.rotate(
    ...
    def set_color(
        self,
        color: ParsableManimColor | Sequence[ParsableManimColor] = YELLOW_C,
        family: bool = True,
    ) -> Self:
    ...
    def set_submobject_colors_by_gradient(self, *colors: ParsableManimColor) -> Self:
        ...
        new_color_or_colors = color_gradient(colors, len(mobs))

        if isinstance(new_color_or_colors, list):
            for mob, color in zip(mobs, new_color_or_colors):
                mob.set_color(color, family=False)
        else:
            mobs[0].set_color(new_color_or_colors, family=False)
        return self
    ...
    def set_submobject_colors_by_radial_gradient(
        ...
        for mob in self.family_members_with_points():
            t = cast(float, np.linalg.norm(mob.get_center() - center) / radius)
            t = min(t, 1)
    ...
    def get_color(self) -> ManimColor | list[ManimColor]:
    ...
    def reduce_across_dimension(
        self,
        reduce_func: Callable[[npt.NDArray[ManimFloat] | Sequence[float]], float],
        dim: int,
    ) -> float:
        ...
        if len(self.points) == 0:  # noqa: SIM108
            ...
            return_val = reduce_func(self.points[:, dim])
        ...
        for mobj in self.submobjects:
            ...
            return_val = (
                value if return_val is None else reduce_func([value, return_val])
            )
    ...
    def get_merged_array(self, array_attr: str) -> np.ndarray:
        ...
        """
        result: np.ndarray = getattr(self, array_attr)
        for submob in self.submobjects:
    ...
    def get_extremum_along_dim(
        ...
        values: npt.NDArray[ManimFloat] = np_points[:, dim]
        if key < 0:
            extremum = np.min(values)
        elif key == 0:
            extremum = (np.min(values) + np.max(values)) / 2
        else:
            extremum = np.max(values)
        return cast(float, extremum)
    ...
    def get_boundary_point(self, direction: Vector3D) -> Point3D:
        ...
        boundary_point: Point3D = all_points[index]
    ...
    def get_coord(self, dim: int, direction: Vector3D = ORIGIN) -> float:
    ...
    def get_pieces(self, n_pieces: int) -> Group:
        ...
        alphas = np.linspace(0.0, 1.0, n_pieces + 1)
        return Group(
            *(
    ...
    def match_dim_size(
        self, mobject: Mobject, dim: int, **kwargs: Unpack[ApplyPointsFunctionKwargs]
    ) -> Self:
        """Match the specified dimension with the dimension of another :class:`~.Mobject`."""
        return self.rescale_to_fit(mobject.length_over_dim(dim), dim, **kwargs)

    def match_width(
        self, mobject: Mobject, **kwargs: Unpack[ApplyPointsFunctionKwargs]
    ) -> Self:
    ...
    def __getitem__(self, value: int | slice) -> Self | Group:
    ...
    def arrange_in_grid(
        ...
        **kwargs: Any,
        ...
        def init_num_cols_or_rows(
            num_cols_or_rows: int | None,
            alignments: str | None,
            sizes: Sequence[float | None] | None,
        ) -> int | None:
            if num_cols_or_rows is not None:
                return num_cols_or_rows
        ...
        cols = init_num_cols_or_rows(cols, col_alignments, col_widths)
        rows = init_num_cols_or_rows(rows, row_alignments, row_heights)

        # Calculate rows and cols.
        if rows is None:
            if cols is None:
                # Make the grid as close to quadratic as possible.
                # Choosing cols first can result in cols > rows.
                # This is favored over rows > cols since, in general,
                # the scene's width is larger than its height.
        ...
        def init_alignments(
            alignment_chars: str | None,
            num_rows_or_cols: int,
            char_to_direction_mapping: dict[str, Vector3D],
            name: str,
            dir_: Vector3D,
        ) -> list[Vector3D]:
            if alignment_chars is None:
                # Use cell_alignment as fallback
                return [cell_alignment * dir_] * num_rows_or_cols
            if len(alignment_chars) != num_rows_or_cols:
                raise ValueError(f"{name}_alignments has a mismatching size.")
            alignment_vectors = [
                char_to_direction_mapping[char] for char in alignment_chars
            ]
            return alignment_vectors

        row_alignment_vectors = init_alignments(
            row_alignments,
            rows,
        ...
        col_alignment_vectors = init_alignments(
            col_alignments,
            cols,
        ...
        @overload
        def reverse(maybe_sequence: list[Vector3D]) -> list[Vector3D]: ...

        @overload
        def reverse(maybe_sequence: Sequence[float | None]) -> list[float | None]: ...

        @overload
        def reverse(maybe_sequence: None) -> None: ...

        def reverse(
            maybe_sequence: list[Vector3D] | Sequence[float | None] | None,
        ) -> list[Vector3D] | list[float | None] | None:
            if maybe_sequence is None:
                return None
            return list(reversed(maybe_sequence))

        reversed_row_alignment_vectors = reverse(row_alignment_vectors)
        reversed_row_heights = reverse(row_heights)
        ...
        def init_heights_or_widths(
            ...
            if sizes is None:
                sizes = [None] * num_rows_or_cols
            if len(sizes) != num_rows_or_cols:
        ...
        heights = init_heights_or_widths(
            reversed_row_heights, rows, measured_heights, "row_heights"
        )
        widths = init_heights_or_widths(col_widths, cols, measured_widths, "col_widths")
        ...
        for r in range(rows):
            ...
            for c in range(cols):
                if grid[r][c] is not placeholder:
                    alignment = (
                        reversed_row_alignment_vectors[r] + col_alignment_vectors[c]
                    )
                    line = Line(
                        x * RIGHT + y * UP,
    ...
    def get_point_mobject(self, center: Point3DLike | None = None) -> Any:
...
def override_animate(
    ...
    def decorator(override_method: OverrideAnimateMethod) -> OverrideAnimateMethod:
        method._override_animate = override_method
